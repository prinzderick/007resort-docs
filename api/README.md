...
6. **Client-supplied ids.** `POST /orders`, `POST /orders/{id}/lines` (line `id`), `POST /tabs` and each tender in `POST /payments` (tender `id`, becomes the Payment id) accept an optional client-generated UUIDv7 `id`, unique per site, plus an informational `clientCreatedAt` (ISO UTC; the server keeps its own `createdAt`). Generate ids at creation time so records can be created and referenced (for example a line on an order) while disconnected, then replayed later. Semantics: the server uses the id if valid and unused; a replay with the same id and same body returns the original resource (same body, no duplicate); the same id with a different body returns `409 concurrency_conflict`; a non-UUIDv7 id returns `422 validation_failed`. Other resources are still server-assigned.
...
8. **Clock.** Send `clientCreatedAt` in the body where supported (or `X-Client-Timestamp` elsewhere) for audit. It is informational; the server uses its own clock for all business decisions.
