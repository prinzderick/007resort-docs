...
| --- | --- | --- |
| **0** | Repositories, architecture docs, database design, API contracts skeleton, dev environment, CI | This document set reviewed and approved; all 8 repos scaffolded with green CI; `007resort-infrastructure/compose/dev` runs MySQL 8.4 locally |
...
| **2** | Catalog, POS transactions, orders, payments, receipt/device abstractions | Concurrency tests C-4/C-5/C-6 pass; `007resort-pos-desktop` v1 processes a real sale end-to-end against a test API |
...
| **4** | Hospitality: tables, tabs, KDS, real-time routing, waiter workflow | `007resort-kds` receives and transitions tickets in real time; routing tests green |
| **5** | `007resort-mobile`: attendants, supervisors, Sports Entrance, Sports Store | Widget tests for each mode; tablet checkout tracked end-to-end |
...
| **7** | `007resort-admin-web`: finance, inventory reporting, staff, configuration, dashboards | Reporting reconciles to transactions/audit; no direct business-table writes from PHP |
| **8** | `007resort-booking-web`: public site, online booking, online payment, customer history | Payment provider integration tested with webhook idempotency (C-4) |
